...
model.load_state_dict(torch.load('mfn_cls_model.pt', map_location=device))  # ⚡ 换成分类版模型
model.eval()
...
with torch.no_grad():
    for (text, audio, visual), labels in dataloader:
        ...
        labels = (labels > 0).long().to(device)  # ⚡ labels也要转成 0/1

        outputs = model(text, audio, visual)  # (B, 2)
        preds = torch.argmax(outputs, dim=1)  # ⚡ 取最大得分作为类别 (B,)
